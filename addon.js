...
function configurationPage() {
  ...
  <h1>Configure Nuvio Streams</h1><p>Enable the sources you want. Results follow this priority from top to bottom.</p><ul id="providers">${rows}</ul><a id="install" class="action" href="#">Install in Stremio</a><button id="copy" class="copy" type="button">Copy manifest URL</button><code id="url"></code>
  <script>const list=document.querySelector('#providers'),url=document.querySelector('#url'),install=document.querySelector('#install'),copy=document.querySelector('#copy');function manifest(){const providers=[...list.children].filter(row=>row.querySelector('input').checked).map(row=>row.dataset.id);return location.origin+'/'+encodeURIComponent(JSON.stringify({providers}))+'/manifest.json'}function stremioUrl(){const parsed=new URL(manifest());return 'stremio://'+parsed.host+parsed.pathname+parsed.search}function refresh(){url.textContent=manifest();install.href=stremioUrl();copy.textContent='Copy manifest URL'}async function copyManifest(){const value=manifest();try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(value)}else{const area=document.createElement('textarea');area.value=value;area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();if(!document.execCommand('copy'))throw new Error('copy failed');area.remove()}copy.textContent='Copied'}catch(error){window.prompt('Copy this manifest URL:',value)}}list.addEventListener('click',event=>{const row=event.target.closest('li');if(!row)return;if(event.target.classList.contains('up')&&row.previousElementSibling)list.insertBefore(row,row.previousElementSibling);if(event.target.classList.contains('down')&&row.nextElementSibling)list.insertBefore(row.nextElementSibling,row);refresh()});list.addEventListener('change',refresh);copy.addEventListener('click',copyManifest);refresh();</script></body></html>`;
}
